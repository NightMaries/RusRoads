...
function parseEdge(so, to, dg, directed, o) {
        ...
        for (const ch of so.children) {
            if (ch.type === 'attr_stmt') {
                for (const drObj of drObjs)
                    fillDrawingObjectAttrs(ch, drObj);
...
function parseGraph(o, dg, directed) {
    parseUnderGraph(o.children, dg, directed);
}
function parseNode(o, dg) {
    const id = o.node_id.id.toString();
    let node = dg.graph.findNode(id);
    let drawingNode;
    if (node) {
        drawingNode = DrawingObject.getDrawingObj(node);
        if (!drawingNode)
...
function fillDrawingObjectAttrs(o, drawingObj) {
    if (o.attr_list == null)
        return;
...
function parseUnderGraph(children, dg, directed) {
        ...
        switch (o.type) {
            ...
            case 'subgraph':
                ...
                if (process_same_rank(o, dg)) {
                    ...
                    const entities = getEntitiesSubg(o, dg, directed);
...
export function parseDotString(graphStr) {
...
function process_same_rank(o, dg) {
...
function styleEnumFromString(t) {
...
function getEntitiesSubg(o, dg, directed) {
    let ret = [];
    for (const ch of o.children) {
        if (ch.type == 'edge_stmt') {
            const edgeList = ch.edge_list;
            for (let i = 0; i < edgeList.length - 1; i++) {
                for (const e of parseEdge(edgeList[i], edgeList[i + 1], dg, directed, ch))
                    ret.push(e);
            }
        }
        else if (ch.type == 'attr_stmt') {
