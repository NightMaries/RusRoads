...
var SegEventType;
(function (SegEventType) {
    SegEventType[SegEventType["VOpen"] = 0] = "VOpen";
    SegEventType[SegEventType["VClose"] = 1] = "VClose";
    SegEventType[SegEventType["HOpen"] = 2] = "HOpen";
})(SegEventType || (SegEventType = {}));
export class SegmentIntersector {
    ...
    constructor() {
        ...
        this.segmentsWithoutVisibility = new Array();
        this.verticalSegmentsScanLine = new RBTree((a, b) => this.CompareSS(a, b));
    ...
    RemoveSegmentsWithNoVisibility(horizontalScanSegments, verticalScanSegments) {
    ...
    ScanIntersect(hSeg) {
        ...
        let segNode = this.verticalSegmentsScanLine.findFirst(this.findFirstPred);
        for (; null != segNode; segNode = this.verticalSegmentsScanLine.next(segNode)) {
            const vSeg = segNode.item;
            if (1 == PointComparer.Compare(vSeg.Start.x, hSeg.End.x)) {
                break;
                //  Out of HSeg range
            }
            const newVertex = this.visGraph.AddVertexP(new Point(vSeg.Start.x, hSeg.Start.y));
    ...
    Compare(first, second) {
        ...
        let cmp = PointComparer.Compare(first.Site.y, second.Site.y);
        if (0 != cmp) {
            return cmp;
        }
        ...
        const vEvent = first.IsVertical ? first : second;
    ...
    CompareSS(first, second) {
        ...
        //  calculations so no additional rounding error is introduced.
        let cmp = PointComparer.Compare(first.Start.x, second.Start.x);
...
//# sourceMappingURL=SegmentIntersector.js.map