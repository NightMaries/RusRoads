...
export class BundleRouter extends Algorithm {
    ...
    OrderOptimizeNudgeEtc() {
        const metroGraphData = new MetroGraphData(this.regularEdges, this.LooseHierarchy, this.TightHierarchy, this.bundlingSettings, this.shortestPathRouter.CdtProperty, this.EdgeLooseEnterable, this.EdgeTightEnterable, this.loosePolylineOfPort);
    ...
    static CreateConstrainedDelaunayTriangulation(looseHierarchy) {
        const obstacles = Array.from(looseHierarchy.GetAllLeaves());
        let rectangle = looseHierarchy.irect;
    ...
    static GetConstrainedDelaunayTriangulation(obstacles) {
    ...
    FigureOutHookLocation(poly, otherEdgeEndPort, edgeGeom) {
        const clusterPort = otherEdgeEndPort;
        if (clusterPort == null) {
    ...
    FigureOutHookLocationForSimpleOtherPort(poly, otherEdgeEndPort, edgeGeom) {
        ...
        // SplineRouter.ShowVisGraph(this.VisibilityGraph, this.LooseHierarchy.GetAllLeaves(),
        //     shapes.Select(sh => sh.BoundaryCurve), new[] { new LineSegment(edgeGeom.SourcePort.Location, edgeGeom.TargetPort.Location) });
        const s = new SingleSourceMultipleTargetsShortestPathOnVisibilityGraph(this.VisibilityGraph.FindVertex(otherEdgeEnd), Array.from(poly).map(this.VisibilityGraph.FindVertex.bind), this.VisibilityGraph);
        const path = s.GetPath();
        for (const sh of shapes) {
            sh.IsTransparent = false;
        }
    ...
    AnalyzeEdgeSeparation() {
        ...
        const es = this.CalculateMaxAllowedEdgeSeparation(pathsOnCdtEdge);
    ...
    CalculateMaxAllowedEdgeSeparation(pathsOnCdtEdge) {
        let l = 0.01;
        let r = 10;
    ...
    EdgeSeparationIsOkMN(pathsOnCdtEdge, separation) {
        // total number of cdt edges
        const total = pathsOnCdtEdge.size;
        if (total == 0) {
            return true;
        }
    ...
    EdgeSeparationIsOk(edge, paths, separation) {
