...
export function layoutGeomGraph(geomGraph, cancelToken) {
    ...
    function layoutEngine(geomGraph, cancelToken) {
        createSettingsIfNeeded(geomGraph);
        if (geomGraph.layoutSettings instanceof SugiyamaLayoutSettings) {
            ...
            const pivotMds = new PivotMDS(geomGraph, cancelToken, () => 1, geomGraph.layoutSettings);
            pivotMds.run();
        }
        else {
...
export function getEdgeRoutingSettingsFromAncestors(geomGraph) {
    do {
...
export function edgeRouter(geomGraph, edgesToRoute, cancelToken) {
    const ers = getEdgeRoutingSettingsFromAncestors(geomGraph);
    if (ers.EdgeRoutingMode == EdgeRoutingMode.Rectilinear) {
        ...
        if (ers.EdgeRoutingMode != EdgeRoutingMode.None) {
            ...
            if (unroutedEdges.length > 0)
                new SplineRouter(geomGraph, unroutedEdges).run();
        }
    }
}
// Lays out a GeomGraph, which is possibly disconnected and might have sub-graphs
export function layoutGeomGraphDetailed(geomG, cancelToken, layoutEngine, edgeRouter, packing, flipToScreenCoords = true) {
        ...
        for (const n of geomG.shallowNodes()) {
            if (n.isGraph) {
                ...
                if (!g.graph.isEmpty()) {
                    ...
                    if (bb && !bb.isEmpty()) {
                        n.boundaryCurve = CurveFactory.mkRectangleWithRoundedCorners(bb.width, bb.height, Math.min(10, bb.width / 10), Math.min(10, bb.height / 10), bb.center);
                    }
                }
            }
        }
...
export function routeRectilinearEdges(geomG, edgesToRoute, cancelToken, nodePadding = 1, cornerFitRadius = 3) {
