...
export function layoutGraphWithSugiayma(geomGraph, cancelToken = null, flipToScreenCoords = true) {
...
export class LayeredLayout extends Algorithm {
    ...
    runPostLayering() {
        const routingSettings = this.sugiyamaSettings.edgeRoutingSettings;
    ...
    StretchWidth() {
        ...
        if (currentSpan.length > GeomConstants.distanceEpsilon) {
            const stretch = desiredSpan.length / currentSpan.length;
            if (stretch > 1) {
    ...
    *NodeAnchors() {
    ...
    CalculateOriginalGraphBox() {
        ...
        const delta = box.leftTop.sub(box.rightBottom).length / 2;
...
function TransformCurves(geometryGraph, m) {
    for (const e of geometryGraph.edges()) {
        if (e.label != null) {
