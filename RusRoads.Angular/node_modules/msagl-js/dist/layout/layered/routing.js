...
export class Routing extends Algorithm {
    ...
    RouteUnroutedEdges() {
        for (const n of this.OriginalGraph.deepNodes()) {
            for (const e of n.outEdges()) {
                if (!e.curve)
                    StraightLineEdges.RouteEdge(e, Math.max(e.source.padding, e.target.padding));
            }
        }
    }
    RouteFlatEdges() {
    ...
    createRegularSplines() {
        for (const intEdgeList of this.Database.RegularMultiedges()) {
            ...
            const optimizeShortEdges = m == 1 && !this.FanAtSourceOrTarget(intEdgeList[0]);
            ...
            if (ip.x == ip.y) {
                ...
                for (const intEdge of v) {
                    ...
                    if (intEdge.edge.label != null) {
                        ...
                        const center = (intEdge.edge.label.center = new Point(c.value((c.parStart + c.parEnd) / 2).x + intEdge.labelWidth / 2, anchor.y));
